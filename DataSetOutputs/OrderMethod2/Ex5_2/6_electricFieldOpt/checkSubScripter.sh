...
then
	...
	then
		...
		echo \# Opt=\(TS, CalcFC, noeigentest, Z-matrix, MaxStep=10\) geom=allcheckpoint guess=read Field=read NoSymm ${func}\/${basis} freq=noraman pop=none >> ${1}_field_N${3}.gjf
		...
		echo \# Opt=\(Z-matrix, MaxStep=10\) ${func}\/${basis} geom=allcheckpoint guess=read Field=read NoSymm freq=noraman pop=none >> ${1}_field_N${3}.gjf
